...
public class JDBCProjectDAO implements ProjectDAO {
	...
	public List<Project> getAllActiveProjects() {
		...
		String sqlActiveProjects = "SELECT project_id, name, from_date, to_date " +
				 				   "FROM project WHERE (now() BETWEEN from_date AND to_date) " +
		...
		while (results.next()) {
			...
			activeProjects.add(projectResult);
		}
	...
	private Project mapRowToProject(SqlRowSet results) {
		...
		specificProject.setId(results.getLong("project_id"));
		specificProject.setName(results.getString("name"));
		if (results.getDate("from_date") != null) {
			specificProject.setStartDate(results.getDate("from_date").toLocalDate());
		}
		if (results.getDate("to_date") != null) {
			specificProject.setEndDate(results.getDate("to_date").toLocalDate());
		}		
