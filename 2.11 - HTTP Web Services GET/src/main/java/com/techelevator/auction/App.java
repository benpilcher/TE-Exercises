...
public class App {
	...
	public static Auction listDetailsForAuction() {
		...
		try {
			int auctionSelected = Integer.parseInt(scanner.nextLine());
			Auction auctionDetails = restTemplate.getForObject(API_URL + "/" + auctionSelected, Auction.class);
	...
	public static Auction[] findAuctionsSearchTitle() {
		...
		try {
			...
			Auction[] auctionsSearchTitle = restTemplate.getForObject(API_URL + "?title_like=" + auctionTitle, Auction[].class);
